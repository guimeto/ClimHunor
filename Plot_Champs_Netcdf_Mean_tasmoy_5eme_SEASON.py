...
########################  OUVERTURE DU CHAMPS A LIRE  ########
        ...
        rep2='anomalie_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmoy_'+period[t]+'_1971-2000_MJ.nc'
        filename=rep1+rep2
...
########################  OUVERTURE DU CHA MPS SIGNIFICATIF  ########
        ...
        rep4='anomalie_ensemble_60percent_significatif_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmoy_'+period[t]+'_1971-2000_MJ.nc'
        filename=rep3+rep4
        nc_fid=Dataset(filename,'r')  
        lats = nc_fid.variables['lat'][:]  # Extrait et copie les données du fichier NetCDF
        lons = nc_fid.variables['lon'][:]
...
##########################################################
    ...
    #    crs=ccrs.PlateCarree()
       ...
       # mycmap.set_over("darkred") 
        ...
        cbar = plt.colorbar(mm,  shrink=0.75, drawedges='True', ticks=np.arange(-8.0, 8.1, 0.5), extend='both',label='°C')
        ...
        plt.xlabel(u'\n\n\nAnomalie brute de Mean_tasmoy (Celcius) ',size='x-large')        
        string_title=u'Anomalie saisonnière de la température moyenne\n  5ème saison Innue (Mai-Juin) RCMs RCP8.5 '+period[t]       
        plt.title(string_title, size='xx-large')       
        plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/Atlas/Atlas_figures/figures_16/anomalie_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmoy_'+period[t]+'_1971-2000_'+Saison[i]+'.png', bbox_inches='tight', pad_inches=0.1)
        plt.show()  
        plt.close()
