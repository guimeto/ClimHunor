...
########################  OUVERTURE DU CHAMPS A LIRE  ########
        rep1='K:/PROJETS/PROJET_CORDEX/CORDEX-NAM44/ANOMALIE_ENSEMBLE/SAISON/Mean_tasmoy/'
        rep2='anomalie_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmoy_'+period[t]+'_1971-2000_'+Saison[i]+'.nc'
        filename=rep1+rep2
        nc_fid=Dataset(filename,'r')  
        lats = nc_fid.variables['lat'][:]  # Extrait et copie les données du fichier NetCDF
        lons = nc_fid.variables['lon'][:]
        time = nc_fid.variables['time'][:]
        Vals = nc_fid.variables['Mean_tasmoy'][:].squeeze() 

########################  OUVERTURE DU CHA MPS SIGNIFICATIF  ########
        rep3='K:/PROJETS/PROJET_CORDEX/CORDEX-NAM44/ANOMALIE_ENSEMBLE/SAISON/SIGNIFICATIF_sigma/Mean_tasmoy/'
        rep4='anomalie_ensemble_60percent_significatif_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmoy_'+period[t]+'_1971-2000_'+Saison[i]+'.nc'
        filename=rep3+rep4
        nc_fid=Dataset(filename,'r')  
        lats = nc_fid.variables['lat'][:]  # Extrait et copie les données du fichier NetCDF
        lons = nc_fid.variables['lon'][:]
        time = nc_fid.variables['time'][:]
        Vals2 = nc_fid.variables['Mean_tasmoy'][:].squeeze() 
...
##########################################################
    ...
    #    crs=ccrs.PlateCarree()
       ...
       # mycmap.set_over("darkred") 
        ...
        cbar = plt.colorbar(mm,  shrink=0.75, drawedges='True', ticks=np.arange(-1.0, 5.1, 0.2), extend='both')
        cbar.ax.tick_params(labelsize=17) 
        plt.xlabel(u'\n\n\nAbsolute anomaly of Mean_tasmoy (Celcius) ',size='x-large')        
        string_title=u'Anomalies saisonnières de la température moyenne journalière (Celcius) \n '+NameFr[i]+'\n  Ensemble de 9 RCMs RCP8.5 ('+period[t]+') / (1971-2000))\n'        
        plt.title(string_title, size='xx-large')       
        plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/Atlas/Atlas_figures/figure_12-15/anomalie_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmoy_'+period[t]+'_1971-2000_'+Saison[i]+'.png', bbox_inches='tight', pad_inches=0.1)
        plt.show()  
        plt.close()
