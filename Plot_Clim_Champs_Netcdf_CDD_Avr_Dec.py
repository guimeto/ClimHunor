...
for s in range(0,1,1):
  for t in range(0,len(period),1):
########################  OUVERTURE DU CHAMPS A LIRE  ########
        rep1='K:/PROJETS/PROJET_CORDEX/CORDEX-NAM44/MOYENNE_ENSEMBLE/Avr_Dec/CDD/'
        rep2='MOYENNE_ENSEMBLE_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_Avr_Dec_'+period[t]+'.nc'
        filename=rep1+rep2
...
##########################################################
    ...
    #    crs=ccrs.PlateCarree()
       ...
       # mycmap = mpl.cm.get_cmap('jet', 40)
        ...
        cbar = plt.colorbar(mm,  shrink=0.75, drawedges='True', ticks=np.arange(10, 21.0, 1.0), extend='both',label='jours')
        ...
        string_title=u'Climatologie du nombre maximal de jours secs consécutifs\n 1er Avril au 1er Décembre RCMs Historiques '+period[t]     
        plt.title(string_title, size='xx-large')       
        plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/Atlas/Atlas_figures/figures_22_23/climatology_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_CDD_'+period[t]+'_Avr_Dec.png', bbox_inches='tight', pad_inches=0.1)
        plt.show()  
        plt.close()
