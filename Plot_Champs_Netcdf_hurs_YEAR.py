...
########################  OUVERTURE DU CHAMPS A LIRE  ########
    rep1='K:/PROJETS/PROJET_CLIMHUNOR/Atlas/netcdf/'
    rep2='ENS_MEAN_hurs_'+period[t]+'.nc'
    filename=rep1+rep2
    nc_fid=Dataset(filename,'r')  
    lats = nc_fid.variables['lat'][:]  # Extrait et copie les données du fichier NetCDF
    lons = nc_fid.variables['lon'][:]
    time = nc_fid.variables['time'][:]
    Vals = nc_fid.variables['hurs'][:].squeeze() 
    ...
    cbar = plt.colorbar(mm,  shrink=0.75, drawedges='True', ticks=np.arange(75, 100.0, 1.0), extend='both',label='%')
    ...
    string_title=u'Climatologie annuelle de l\'humidité relative \n RCMs RCP8.5 '+period[t]        
    plt.title(string_title, size='xx-large')       
    plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/Atlas/Atlas_figures/figures_Ann_Everitt/HR_YEAR/climatology_ensemble_'+nb_model[0]+'_RCMs_Pilotes_GCMs_'+scenario[0]+'_CORDEX_NAM44_ll_hurs_'+period[t]+'_YEAR.png', bbox_inches='tight', pad_inches=0.1)
    plt.show()  
    plt.close()
