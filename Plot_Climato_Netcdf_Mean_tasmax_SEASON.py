...
########################  OUVERTURE DU CHAMPS A LIRE  ########
        rep1='K:/PROJETS/PROJET_CORDEX/CORDEX-NAM44/MOYENNE_ENSEMBLE/SAISON/Mean_tasmax/'
        rep2='MOYENNE_ENSEMBLE_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_Mean_tasmax_'+period[t]+'_'+str(i)+'.nc'
        filename=rep1+rep2
...
##########################################################
    ...
    #    crs=ccrs.PlateCarree()
       ...
       # mycmap = mpl.cm.get_cmap('jet', 40)
        ...
        cbar = plt.colorbar(mm,  shrink=0.75, drawedges='True', ticks=np.arange(10, 30.0, 1.0), extend='both',label='°C')
        ...
        string_title=u'Climatologie saisonnière de la température maximale \n en été '+period[t]+' Historiques \n'        
        plt.title(string_title, size='xx-large')  
        plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/Atlas/Atlas_figures/figures_Ann_Everitt/Tmax_JJA/climatology_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmax_'+period[t]+'_'+str(i)+'.png', bbox_inches='tight', pad_inches=0.1)
        plt.show()  
        plt.close()
