...
########################  OUVERTURE DU CHAMPS A LIRE  ########
        rep1='K:/PROJETS/PROJET_CORDEX/CORDEX-NAM44/ANOMALIE_ENSEMBLE/MOIS/Mean_tasmax/'
        rep2='anomalie_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmax_'+period[t]+'_1971-2000_'+str(i)+'.nc'
        filename=rep1+rep2
...
########################  OUVERTURE DU CHA MPS SIGNIFICATIF  ########
        rep3='K:/PROJETS/PROJET_CORDEX/CORDEX-NAM44/ANOMALIE_ENSEMBLE/MOIS/SIGNIFICATIF_sigma/Mean_tasmax/'
        rep4='anomalie_ensemble_60percent_significatif_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM-44_ll_Mean_tasmax_'+period[t]+'_1971-2000_'+str(i)+'.nc'
        filename=rep3+rep4
        nc_fid=Dataset(filename,'r')  
        lats = nc_fid.variables['lat'][:]  # Extrait et copie les données du fichier NetCDF
        lons = nc_fid.variables['lon'][:]
        time = nc_fid.variables['time'][:]
        Vals2 = nc_fid.variables['Mean_tasmax'][:].squeeze() 
...
##########################################################
       ...
       # mycmap.set_under("b")
        ...
        ax.gridlines()
        fig.canvas.draw()                   
        cs = ax.plot(BV_border1.lon,BV_border1.lat, transform=ccrs.PlateCarree(), color='b', linewidth=4, label='Nitassinan Pessamit')
        ...
        cbar = plt.colorbar(mm,  shrink=0.75, drawedges='True', ticks=np.arange(-10, 10.0, 0.5), extend='both',label='°C')
        ...
        string_title=u'Anomalie de température maximale en janvier\n 2041-2070 RCP8.5'        
        plt.title(string_title, size='xx-large')       
        plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/Atlas/Atlas_figures/figure4/anomalie_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmax_'+period[t]+'_1971-2000_'+str(i)+'.png', bbox_inches='tight', pad_inches=0.1)
        plt.show()  
        plt.close()
