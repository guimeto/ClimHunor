...
########################  OUVERTURE DU CHAMPS A LIRE  ########
        rep1='K:/PROJETS/PROJET_CLIMHUNOR/DRY/'
        rep2='MEAN_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_DC_'+period[t]+'.nc'
        filename=rep1+rep2
        nc_fid=Dataset(filename,'r')  
        lats = nc_fid.variables['lat'][:]  # Extrait et copie les données du fichier NetCDF
        lons = nc_fid.variables['lon'][:]
        time = nc_fid.variables['time'][:]
        Vals = nc_fid.variables['DC'][:].squeeze() 
...
##########################################################
    ...
    #    crs=ccrs.PlateCarree()
       ...
       # mycmap = mpl.cm.get_cmap('jet', 40)
        ...
        cbar = plt.colorbar(mm,  shrink=0.75, drawedges='True', ticks=np.arange(0, 120.0, 5.0), extend='both',label='nb')
        ...
        string_title=u'Climatologie de l\'indice de sécheresse DC-SOPFEU  \n  RCMs RCP8.5 ' +period[t]      
        plt.title(string_title, size='xx-large')       
        plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/Atlas/Atlas_figures/Figures_DC_SOPFEU/climatology_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_DC_'+period[t]+'.png', bbox_inches='tight', pad_inches=0.1)
        plt.show()  
        plt.close()
