...
for s in list_saison:
    ...
    plt.plot(result.index.year, result['mean_histo'][:],  label='Historique', linewidth=2, c=color[0])
    plt.plot(result.index.year, result['mean_rcp45'][:],  label='RCP4.5', linewidth=2, c=color[1])
    plt.plot(result.index.year, result['mean_rcp85'][:],  label='RCP8.5', linewidth=2, c=color[2])
    ...
    if s == 'DJF':        
        title='Anomalies saisonnières de la température minimale en hiver\n '
    elif s == 'MAM':        
        title='Anomalies saisonnières de la température minimale au printemps\n '
    elif s == 'JJA':        
        title='Anomalies saisonnières de la température minimale en été\n '
    elif s == 'SON':        
        title='Anomalies saisonnières de la température minimale en automne\n '
    
    
    plt.title(title, fontsize=20, color='black', weight='semibold')
    # ajout du deuxieme axe
    ax1 = ax.twinx()
    ...
    medians[0] = np.NaN
    median_labels = [str(np.round(s, 1)) for s in medians]
    pos = range(len(medians))
    i=0
    for tick,label in zip(pos,ax2.get_xticklabels()):
        ax2.text(pos[tick], medians[tick] + 0.1, median_labels[tick], 
                horizontalalignment='center', fontsize=20, color = color[i], weight='semibold')
        i+=1
    ...
    plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/Atlas/Atlas_figures/figures_17_20/VI_SEASON_'+s+'_Mean_tasmin.png', bbox_inches='tight', format='png', dpi=1000)
    plt.show()  
    plt.close()
