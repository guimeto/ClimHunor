...
Y=np.array([[153,102,255],[102,51,204],[51,0,153],[0,51,153],[51,102,204],[102,153,255],\
...
########################  OUVERTURE DU CHAMPS A LIRE  ########
        rep1='K:/PROJETS/PROJET_CORDEX/CORDEX-NAM44/MOYENNE_ENSEMBLE/SAISON/Mean_tasmin/'
        rep2='MOYENNE_ENSEMBLE_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_Mean_tasmin_'+period[t]+'_'+str(i)+'.nc'
        filename=rep1+rep2
        nc_fid=Dataset(filename,'r')  
        lats = nc_fid.variables['lat'][:]  # Extrait et copie les données du fichier NetCDF
        lons = nc_fid.variables['lon'][:]
        time = nc_fid.variables['time'][:]
        Vals = nc_fid.variables['Mean_tasmin'][:].squeeze() 
...
##########################################################
    ...
    #    crs=ccrs.PlateCarree()
       ...
       # mycmap = mpl.cm.get_cmap('jet', 40)
        ...
        cbar = plt.colorbar(mm,  shrink=0.75, drawedges='True', ticks=np.arange(0, 30.0, 1.0), extend='both')
        cbar.ax.tick_params(labelsize=17) 
        
        plt.xlabel(u'\n\n\nClimatology of Mean_tasmin (Celcius) ',size='x-large')        
        string_title=u'Climatologie saisonnière de la température minimale journalière (Celcius) \n Été \n  Ensemble de '+nb_model[s]+' RCMs Historiques ('+period[t]+'))\n'        
        plt.title(string_title, size='xx-large')       
        plt.savefig('K:/PROJETS/PROJET_CLIMHUNOR/Atlas/Atlas_figures/figures_Ann_Everitt/Tmin_JJA/climatology_ensemble_'+nb_model[s]+'_RCMs_Pilotes_GCMs_'+scenario[s]+'_CORDEX_NAM44_ll_Mean_tasmin_'+period[t]+'_'+str(i)+'.png', bbox_inches='tight', pad_inches=0.1)
        plt.show()  
        plt.close()
